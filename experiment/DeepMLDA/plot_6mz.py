...
model.load_state_dict(torch.load('./deepmlda6m.pth'))
...
#データセット定義
ds_tr = TensorDataset(torch.from_numpy(tr_x1).float(),torch.from_numpy(tr_x2).float(),torch.from_numpy(tr_x3).float(),torch.from_numpy(tr_x4).float(),torch.from_numpy(tr_x5).float(),torch.from_numpy(tr_x6).float(),torch.from_numpy(tr_label).int())
ds_te = TensorDataset(torch.from_numpy(te_x1).float(),torch.from_numpy(te_x2).float(),torch.from_numpy(te_x3).float(),torch.from_numpy(te_x4).float(),torch.from_numpy(te_x5).float(),torch.from_numpy(te_x6).float(),torch.from_numpy(te_label).int())
...
for x,t in enumerate(trainloader):
    ...
    #print(f"te_x1 ->{t[0]}")
    #print(f"te_x1 ->{t[1]}")
    mean, logvar, jmvae_x1_recon, x1_recon, x1_mean, x1_logvar, jmvae_x2_recon, x2_recon, x2_mean, x2_logvar, jmvae_x3_recon, x3_recon, x3_mean, x3_logvar, jmvae_x4_recon, x4_recon, x4_mean, x4_logvar, jmvae_x5_recon, x5_recon, x5_mean, x5_logvar, jmvae_x6_recon, x6_recon, x6_mean, x6_logvar, z_hoge = model(t[0], t[1], t[2], t[3], t[4], t[5])
    tr_z = z_hoge.cpu()
    tr_label = t[6].cpu()
