...
from ptavitm.model_5m import train
...
def main(cuda,batch_size,epochs,top_words,testing_mode,k):#上のコマンドライン引数
    ...
    x5_reverse_vocab = {x5_vocab[word]: word for word in x4_vocab}
    x5_indexed_vocab = [x5_reverse_vocab[index] for index in range(len(x5_reverse_vocab))]
    ds_tr = TensorDataset(torch.from_numpy(tr_x1).float(),torch.from_numpy(tr_x2).float(),torch.from_numpy(tr_x3).float(),torch.from_numpy(tr_x4).float(), torch.from_numpy(tr_x5).float())
    ds_te = TensorDataset(torch.from_numpy(te_x1).float(),torch.from_numpy(te_x2).float(),torch.from_numpy(te_x3).float(),torch.from_numpy(te_x4).float(), torch.from_numpy(te_x5).float())
