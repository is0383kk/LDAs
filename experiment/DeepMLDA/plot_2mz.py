...
model.load_state_dict(torch.load('./deepmlda2m.pth'))
...
ds_te = TensorDataset(torch.from_numpy(te_x1).float(), torch.from_numpy(te_x2).float(),torch.from_numpy(te_label).int())
