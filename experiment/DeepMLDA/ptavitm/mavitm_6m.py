...
class MAVITM(nn.Module):
    def __init__(self,
        ...
        self.inference = joint_encoder(joint_input, 250, 200, encoder_noise)
        self.mean = hidden(200, topics, eps=batchnorm_eps, momentum=batchnorm_momentum)
        self.logvar = hidden(200, topics, eps=batchnorm_eps, momentum=batchnorm_momentum)
        ...
        # Inference net q(z|x5) & Generator p(x5|z)
        self.inference_x5 = encoder(input_x5, hidden1_dimension, hidden2_dimension, encoder_noise)
        self.x5_mean = hidden(hidden2_dimension, topics, eps=batchnorm_eps, momentum=batchnorm_momentum)
        self.x5_logvar = hidden(hidden2_dimension, topics, eps=batchnorm_eps, momentum=batchnorm_momentum)
        self.x5_generator = decoder(
            input_x5, topics, decoder_noise=decoder_noise, eps=batchnorm_eps, momentum=batchnorm_momentum
        )
        # Inference net q(z|x6) & Generator p(x6|z)
        self.inference_x6 = encoder(input_x6, hidden1_dimension, hidden2_dimension, encoder_noise)
        self.x6_mean = hidden(hidden2_dimension, topics, eps=batchnorm_eps, momentum=batchnorm_momentum)
        self.x6_logvar = hidden(hidden2_dimension, topics, eps=batchnorm_eps, momentum=batchnorm_momentum)
        self.x6_generator = decoder(
            input_x6, topics, decoder_noise=decoder_noise, eps=batchnorm_eps, momentum=batchnorm_momentum
        )
        ...
        for component in [self.mean, self.logvar,
                          ...
                          self.x5_mean, self.x5_logvar, self.x5_generator,
                          self.x6_mean, self.x6_logvar, self.x6_generator
                          ]:
        ...
        nn.init.xavier_uniform_(self.inference_x5.linear1.weight, gain=1)
        nn.init.xavier_uniform_(self.inference_x6.linear1.weight, gain=1)
        if word_embeddings is not None:
            ...
            copy_embeddings_(self.inference_x5.linear1.weight, word_embeddings)
            copy_embeddings_(self.inference_x6.linear1.weight, word_embeddings)
        if not train_word_embeddings:
            ...
            self.inference_x5.linear1.weight.requires_grad = False
            self.inference_x5.linear1.bias.requires_grad = False
            self.inference_x5.linear1.bias.fill_(0.0)
            self.inference_x6.linear1.weight.requires_grad = False
            self.inference_x6.linear1.bias.requires_grad = False
            self.inference_x6.linear1.bias.fill_(0.0)
        ...
        nn.init.xavier_uniform_(self.x5_generator.linear.weight, gain=1)
        nn.init.xavier_uniform_(self.x6_generator.linear.weight, gain=1)

    def joint_encode(self, x1_batch, x2_batch, x3_batch, x4_batch, x5_batch, x6_batch):
        """
        同時分布を求める推論ネットワーク用
        """
        encoded = self.inference(torch.cat([x1_batch, x2_batch, x3_batch, x4_batch, x5_batch, x6_batch], 1))
    ...
    def inferenceX5(self, batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        x5の推論ネットワーク
        """
        encoded = self.inference_x5(batch)
        return encoded, self.x5_mean(encoded), self.x5_logvar(encoded)


    def generatorX5(self, mean: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
        # Same as _generator_network method in the original TensorFlow implementation at https://git.io/fhUJu
        """
        x5の生成ネットワーク
        """
        ...
        z = mean + logvar.exp().sqrt() * eps
        z = F.softmax(z,dim=1)
        return self.x5_generator(z)

    def inferenceX6(self, batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        x6の推論ネットワーク
        """
        encoded = self.inference_x6(batch)
        return encoded, self.x6_mean(encoded), self.x6_logvar(encoded)


    def generatorX6(self, mean: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
        # Same as _generator_network method in the original TensorFlow implementation at https://git.io/fhUJu
        """
        x6の生成ネットワーク
        """
        ...
        z = mean + logvar.exp().sqrt() * eps
        z = F.softmax(z,dim=1)
        return self.x6_generator(z)

    def sample_z(self,mean,logvar): # 独自で定義,潜在変数の可視化に必要
        eps = mean.new().resize_as_(mean).normal_(mean=0, std=1)
        return mean + logvar.exp().sqrt() * eps

    def forward(self, x1_batch, x2_batch, x3_batch, x4_batch, x5_batch, x6_batch):
        _, mean, logvar = self.joint_encode(x1_batch, x2_batch, x3_batch, x4_batch, x5_batch, x6_batch) # 同時分布の平均と分散
        ...
        _, x5_mean, x5_logvar = self.inferenceX5(x5_batch) # x5モダリティの平均と分散
        _, x6_mean, x6_logvar = self.inferenceX6(x6_batch) # x5モダリティの平均と分散
        ...
        jmvae_x5_recon = self.generatorX5(mean, logvar) # 同時分布から生成したx5モダリティの復元誤差
        jmvae_x6_recon = self.generatorX6(mean, logvar) # 同時分布から生成したx5モダリティの復元誤差
        ...
        x5_recon = self.generatorX5(x5_mean, x5_logvar) # 同時分布から生成したx4モダリティの復元誤差
        x6_recon = self.generatorX6(x6_mean, x6_logvar) # 同時分布から生成したx4モダリティの復元誤差
        z_hoge = self.sample_z(mean, logvar)
        return mean, logvar, jmvae_x1_recon, x1_recon, x1_mean, x1_logvar, jmvae_x2_recon, x2_recon, x2_mean, x2_logvar, jmvae_x3_recon, x3_recon, x3_mean, x3_logvar, jmvae_x4_recon, x4_recon, x4_mean, x4_logvar, jmvae_x5_recon, x5_recon, x5_mean, x5_logvar, jmvae_x6_recon, x6_recon, x6_mean, x6_logvar, z_hoge
    ...
    def telbo(self,
             ...
             x5_batch: torch.Tensor,
             x6_batch: torch.Tensor,
             mean: torch.Tensor,
             logvar: torch.Tensor,
             ...
             jmvae_x5_recon: torch.Tensor,
             x5_recon: torch.Tensor,
             x5_mean: torch.Tensor,
             x5_logvar: torch.Tensor,
             jmvae_x6_recon: torch.Tensor,
             x6_recon: torch.Tensor,
             x6_mean: torch.Tensor,
             x6_logvar: torch.Tensor,
             lambda_x1234: float,
             lambda_x1: float,
             lambda_x2: float,
             lambda_x3: float,
        ...
        jmvae_x5_rl = -(x5_batch * (jmvae_x5_recon + 1e-10).log()).sum(1)
        jmvae_x6_rl = -(x6_batch * (jmvae_x6_recon + 1e-10).log()).sum(1)
        ...
        x5_rl = -(x5_batch * (x5_recon + 1e-10).log()).sum(1)
        x6_rl = -(x6_batch * (x6_recon + 1e-10).log()).sum(1)
        ##############################################################################################
        # 同時分布と事前分布とのKL計算
        ...
        #############################################################################################
        x5_prior_mean = self.prior_mean.expand_as(x5_mean)
        x5_prior_var = self.prior_var.expand_as(x5_logvar)
        x5_prior_logvar = self.prior_logvar.expand_as(x5_logvar)
        x5_var_division = x5_logvar.exp() / x5_prior_var # Σ_0 / Σ_1
        x5_diff = x5_mean - x5_prior_mean # μ_１ - μ_0
        x5_diff_term = x5_diff *x5_diff / x5_prior_var # (μ_1 - μ_0)(μ_1 - μ_0)/Σ_1
        x5_logvar_division = x5_prior_logvar - x5_logvar # log|Σ_1| - log|Σ_0| = log(|Σ_1|/|Σ_2|)
        # KL
        x5_kld = 0.5 * ((x5_var_division + x5_diff_term + x5_logvar_division).sum(1) - self.topics)
        ##############################################################################################
        x6_prior_mean = self.prior_mean.expand_as(x6_mean)
        x6_prior_var = self.prior_var.expand_as(x6_logvar)
        x6_prior_logvar = self.prior_logvar.expand_as(x6_logvar)
        x6_var_division = x6_logvar.exp() / x6_prior_var # Σ_0 / Σ_1
        x6_diff = x6_mean - x6_prior_mean # μ_１ - μ_0
        x6_diff_term = x6_diff *x6_diff / x6_prior_var # (μ_1 - μ_0)(μ_1 - μ_0)/Σ_1
        x6_logvar_division = x6_prior_logvar - x6_logvar # log|Σ_1| - log|Σ_0| = log(|Σ_1|/|Σ_2|)
        # KL
        x6_kld = 0.5 * ((x6_var_division + x6_diff_term + x6_logvar_division).sum(1) - self.topics)
        ##############################################################################################

        # JMVAE_loss, TELBO_lossの第１項
        jmvae_zero_loss = kld + lambda_x1234 * (jmvae_x1_rl + jmvae_x2_rl + jmvae_x3_rl + jmvae_x4_rl+ jmvae_x5_rl+jmvae_x6_rl) # Equation (3) of paper of JMVAE
        x1_elbo = x1_kld + lambda_x1 * x1_rl
        x2_elbo = x2_kld + lambda_x2 * x2_rl
        x3_elbo = x3_kld + lambda_x3 * x3_rl
        x4_elbo = x4_kld + lambda_x4 * x4_rl
        x5_elbo = x5_kld + lambda_x5 * x5_rl
        x6_elbo = x6_kld + lambda_x6 * x6_rl
        return jmvae_zero_loss + x1_elbo + x2_elbo + x3_elbo + x4_elbo + x5_elbo + x6_elbo
