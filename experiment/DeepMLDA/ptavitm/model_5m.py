...
def train(dataset: torch.utils.data.Dataset,
    ...
    for epoch in range(epochs):
        ...
        for index, batch in enumerate(data_iterator):
            ...
            if cuda:
                ...
                x3_batch = x3_batch.cuda(non_blocking=True)
                x4_batch = x4_batch.cuda(non_blocking=True)
                x5_batch = x5_batch.cuda(non_blocking=True)
            # run the batch through the autoencoder and obtain the output
            if corruption is not None:
                mean, logvar, jmvae_x1_recon, x1_recon, x1_mean, x1_logvar, jmvae_x2_recon, x2_recon, x2_mean, x2_logvar, jmvae_x3_recon, x3_recon, x3_mean, x3_logvar, jmvae_x4_recon, x4_recon, x4_mean, x4_logvar,jmvae_x5_recon, x5_recon, x5_mean, x5_logvar, z_hoge = autoencoder(F.dropout(x1_batch, x2_batch, x3_batch, x4_batch, x5_batch, corruption))
            else:
                mean, logvar, jmvae_x1_recon, x1_recon, x1_mean, x1_logvar, jmvae_x2_recon, x2_recon, x2_mean, x2_logvar, jmvae_x3_recon, x3_recon, x3_mean, x3_logvar, jmvae_x4_recon, x4_recon, x4_mean, x4_logvar, jmvae_x5_recon, x5_recon, x5_mean, x5_logvar, z_hoge = autoencoder(x1_batch, x2_batch, x3_batch, x4_batch, x5_batch)
    ...
    torch.save(autoencoder.state_dict(), 'deepmlda5m.pth')
