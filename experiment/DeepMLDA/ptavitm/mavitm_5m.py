...
class MAVITM(nn.Module):
    ...
    def joint_encode(self, x1_batch, x2_batch, x3_batch, x4_batch, x5_batch):
        """
        同時分布を求める推論ネットワーク用
        """
        encoded = self.inference(torch.cat([x1_batch, x2_batch, x3_batch, x4_batch, x5_batch], 1))
    ...
    def forward(self, x1_batch, x2_batch, x3_batch, x4_batch, x5_batch):
        _, mean, logvar = self.joint_encode(x1_batch, x2_batch, x3_batch, x4_batch, x5_batch) # 同時分布の平均と分散
        ...
        return mean, logvar, jmvae_x1_recon, x1_recon, x1_mean, x1_logvar, jmvae_x2_recon, x2_recon, x2_mean, x2_logvar, jmvae_x3_recon, x3_recon, x3_mean, x3_logvar, jmvae_x4_recon, x4_recon, x4_mean, x4_logvar, jmvae_x5_recon, x5_recon, x5_mean, x5_logvar, z_hoge
    ...
    def telbo(self,
        ...
        jmvae_zero_loss = kld + lambda_x1234 * (jmvae_x1_rl + jmvae_x2_rl + jmvae_x3_rl + jmvae_x4_rl+ jmvae_x5_rl) # Equation (3) of paper of JMVAE
        ...
        return jmvae_zero_loss + x1_elbo + x2_elbo + x3_elbo + x4_elbo + x5_elbo
