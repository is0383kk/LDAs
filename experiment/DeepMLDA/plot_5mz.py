...
from ptavitm.mavitm_5m import MAVITM
# データローダ
...
model = MAVITM(
topics = define_topic,
joint_input = len(tr_x1[0]) + len(tr_x2[0]) + len(tr_x3[0]) + len(tr_x4[0]) + len(tr_x5[0]),
...
model.load_state_dict(torch.load('./deepmlda5m.pth'))
...
#データセット定義
ds_tr = TensorDataset(torch.from_numpy(tr_x1).float(),torch.from_numpy(tr_x2).float(),torch.from_numpy(tr_x3).float(),torch.from_numpy(tr_x4).float(),torch.from_numpy(tr_x5).float(),torch.from_numpy(tr_label).int())
ds_te = TensorDataset(torch.from_numpy(te_x1).float(),torch.from_numpy(te_x2).float(),torch.from_numpy(te_x3).float(),torch.from_numpy(te_x4).float(),torch.from_numpy(te_x5).float(),torch.from_numpy(te_label).int())
...
def visualize_zs(zs, labels, mode, ari):
    ...
    for p, l in zip(points, labels):
        ...
        plt.scatter(p[0], p[1], marker="${}$".format(l),s=100)
...
for x,t in enumerate(trainloader):
    ...
    #print(f"te_x1 ->{t[0]}")
    #print(f"te_x1 ->{t[1]}")
    mean, logvar, jmvae_x1_recon, x1_recon, x1_mean, x1_logvar, jmvae_x2_recon, x2_recon, x2_mean, x2_logvar, jmvae_x3_recon, x3_recon, x3_mean, x3_logvar, jmvae_x4_recon, x4_recon, x4_mean, x4_logvar, jmvae_x5_recon, x5_recon, x5_mean, x5_logvar, z_hoge = model(t[0], t[1], t[2], t[3], t[4])
    tr_z = z_hoge.cpu()
    tr_label = t[5].cpu()
