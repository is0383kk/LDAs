...
def main(topic_n,
	...
	print(f"hist{hist}")
	hist = hist / VOCABULARY_SIZE # 正規化
	print(f"hist{hist}")
	if test == False:
		np.savetxt( "k"+str(topic_n)+"trc.txt", hist, fmt=str("%2f") )
		np.savetxt( "k"+str(topic_n)+"trc_label.txt", document_label, fmt=str("%d") )
	else:
		np.savetxt( "k"+str(topic_n)+"tec.txt", hist, fmt=str("%d") )
		np.savetxt( "k"+str(topic_n)+"tec_label.txt", document_label, fmt=str("%d") )
