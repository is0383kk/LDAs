...
from ptavitm.model2 import train
...
# ここから本番######################################################################
    ...
    def visualize_zs_train(zs, labels):
        ...
        plt.savefig('./sample_z/'+'nn2TRAIN'+'k'+str(define_topic)+'v'+str(hist.shape[1])+'d'+str(hist.shape[0])+'.png')
    ...
    def visualize_zs_test(zs, labels):
        ...
        for p, l in zip(points, labels):
            ...
            plt.scatter(p[0], p[1], marker="${}$".format(l),c=colors[l],s=100)
            #ax.scatter(p[0], p[1], p[2], marker="${}$".format(l),c=colors[l])
        plt.savefig('./sample_z/'+'nn2TEST'+'k'+str(define_topic)+'v'+str(test_hist.shape[1])+'d'+str(test_hist.shape[0])+'.png')
