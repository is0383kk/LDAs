...
def visualize_zs_train(zs, labels):
    ...
    ax = Axes3D(fig)
    points = TSNE(n_components=3, random_state=0).fit_transform(zs)
    ...
    plt.savefig('./sample_z/'+'3DTRAIN'+'k'+str(define_topic)+'v'+str(hist.shape[1])+'d'+str(hist.shape[0])+'.png')
...
def visualize_zs_test(zs, labels):
    ...
    for p, l in zip(points, labels):
        ...
        #plt.scatter(p[0], p[1], marker="${}$".format(l),c=colors[l],s=60)
        ax.scatter(p[0], p[1], p[2], marker="${}$".format(l),c=colors[l])
    plt.savefig('./sample_z/'+'3DTEST'+'k'+str(define_topic)+'v'+str(test_hist.shape[1])+'d'+str(test_batch)+'.png')
