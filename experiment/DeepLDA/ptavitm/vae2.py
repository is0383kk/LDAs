...
def hidden(hidden1_dimension: int,
           topics: int,
           eps: float,
           momentum: float) -> torch.nn.Module:
    return nn.Sequential(OrderedDict([
        ('linear', nn.Linear(hidden1_dimension, topics)),
...
class ProdLDA(nn.Module):
    def __init__(self,
        ...
        self.topics = topics
        self.encoder = encoder(in_dimension, hidden1_dimension, encoder_noise)
        self.mean = hidden(hidden1_dimension, topics, eps=batchnorm_eps, momentum=batchnorm_momentum)
        self.logvar = hidden(hidden1_dimension, topics, eps=batchnorm_eps, momentum=batchnorm_momentum)
