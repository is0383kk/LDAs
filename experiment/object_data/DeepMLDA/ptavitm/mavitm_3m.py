...
class MAVITM(nn.Module):
    def __init__(self,
        ...
        self.inference = joint_encoder(joint_input, 200, 200, encoder_noise)
        ...
        # 事前分布のパラメータを定義
        self.prior_mean, self.prior_var = map(nn.Parameter, prior(topics, 1.0))
    ...
    def joint_encode(self, x1_batch, x2_batch, x3_batch):
        """
        同時分布を求める推論ネットワーク用
        """
        encoded = self.inference(torch.cat([x1_batch, x2_batch, x3_batch], 1))
    ...
    def forward(self, x1_batch, x2_batch, x3_batch):
        _, mean, logvar = self.joint_encode(x1_batch, x2_batch, x3_batch) # 同時分布の平均と分散
        ...
        return mean, logvar, jmvae_x1_recon, x1_recon, x1_mean, x1_logvar, jmvae_x2_recon, x2_recon, x2_mean, x2_logvar, jmvae_x3_recon, x3_recon, x3_mean, x3_logvar, z_hoge
    ...
    def telbo(self,
        ...
        jmvae_zero_loss = kld + lambda_x123 * (jmvae_x1_rl + jmvae_x2_rl + jmvae_x3_rl) # Equation (3) of paper of JMVAE
        x1_elbo = x1_kld + lambda_x1 * x1_rl
        x2_elbo = x2_kld + lambda_x2 * x2_rl
        x3_elbo = x3_kld + lambda_x3 * x3_rl
        return jmvae_zero_loss + x1_elbo + x2_elbo + x3_elbo
    ...
    def jmvae_zero_loss(self,
        ...
        ##############################################################################################
        # JMVAE_loss
        jmvae_zero_loss = (kld + x1_rl + x2_rl + x3_rl) # Equation (3) of paper
    ...
    def kl_x123(self,
        ...
        return x1_kld + x2_kld + x3_kld

    def jmvae_kl_loss(self,
             jmvae_zero_loss: torch.Tensor,
             kl_x123: torch.Tensor,
             alpha: float,
             ) -> torch.Tensor:
        loss = jmvae_zero_loss + (alpha * kl_x123)
        return loss
