...
def mlda( data, label , K, num_itr=epoch_num, save_dir="model", load_dir=None ):
    ...
    for it in range(num_itr):
        ...
        print(f"ARI:{ari}, ACC:{acc}, ACCI:{acci}")
...
def main():
    ...
    data = []
    label = []
    data.append( np.loadtxt( "./word.txt" , dtype=np.int32) )
    data.append( np.loadtxt( "./tactile.txt" , dtype=np.int32) )
    data.append( np.loadtxt( "./audio.txt" , dtype=np.int32) )
    data.append( np.loadtxt( "./vision.txt" , dtype=np.int32) )
    label.append(np.loadtxt( "./label.txt" , dtype=np.int32))
    #for i in range(30):
    #mlda( data, label , topic, 1, "learn_result" )

    #data[1] = None
    mlda( data, label , topic, 100, "learn_result" , "learn_result" )
